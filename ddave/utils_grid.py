...
class Player(Dynamic):
    ...
    def updatePosition(self, player_x, player_y, level, screen_max_height):  
        ...
        ## Move X: START
        player_newx = player_x + 1 * self.direction_x.value                   # Tries to walk to the direction the player's going
        ...
        # print(player_newy, self.velocity_y)
...
class Enemy(Dynamic):
    ...
    def __init__(self, x, y, width, height, hitbox_width, hitbox_height, animation_delay, 
                 shot_frequency, shot_chance, movement_type, speed, gfx_id):
        super().__init__(x, y, width, height, hitbox_width, hitbox_height, animation_delay, gfx_id)
    ...
    def move(self, player_position_x, player_position_y):
        # Implement enemy movement logic based on movement type
        pass
    ...
    def update(self, player_position_x, player_position_y):
        self.move(player_position_x, player_position_y)
        if random() < self.shot_chance:
            self.shoot()
