...
class Args:
    ...
    num_iterations_obs_norm_init: int = 4
...
if __name__ == "__main__":
    ...
    envs = SubprocVecEnv([lambda : FrameStack(DangerousDaveEnv(), 4) for _ in range(args.num_envs)])
    ...
    for step in range(args.num_steps * args.num_iterations_obs_norm_init):
        if step % args.num_steps == 0:
            print(f"Step {step} of {args.num_steps * args.num_iterations_obs_norm_init}")
        acs = np.random.randint(0, envs.single_action_space.n, size=(args.num_envs,))
        s, r, d, _ = envs.step(acs)
        next_ob += s[:, 3, :, :].reshape([-1, 1, 96, 60]).tolist()
        # next_ob += s.reshape([-1, 1, 96, 60]).tolist()
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            rnd_next_obs = (
                (
                    (next_obs[:, 3, :, :].reshape(args.num_envs, 1, 96, 60) - torch.from_numpy(obs_rms.mean).to(device))
            ...
            for idx, d in enumerate(done):
                if d:
                    avg_returns.append(envs.returned_episode_returns[idx])
